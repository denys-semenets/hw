  ...
  products_enriched AS (
    SELECT
      ...
      10 + MOD(ABS(FARM_FINGERPRINT(product_id)), 15) AS current_price
...
CREATE TABLE IF NOT EXISTS prom_dim.DIM_ORDER AS
SELECT
  ROW_NUMBER() OVER(ORDER BY order_id) order_key, 
...
FROM prom_stage.stage_orders;
  ...
  CREATE TABLE IF NOT EXISTS prom_dim.DIM_CUSTOMER AS 
  SELECT
...
WITH sales_year AS (
  ...
  FROM prom_dim.FACT_SALES fs
  JOIN prom_dim.DIM_DATETIME dd
    ON fs.date_key_FK = dd.date_key
  GROUP BY dd.year_number, fs.customer_key_FK
),
sessions_year AS (
  SELECT
    dd.year_number,
  ...
  FROM prom_dim.FACT_USER_SESSIONS fus
  JOIN prom_dim.DIM_DATETIME dd
    ON fus.date_key_FK = dd.date_key
  GROUP BY dd.year_number, fus.customer_key_FK
)
SELECT
  s.year_number,
  s.customer_key_FK,
  s.total_orders,
  s.total_spend,
  SAFE_DIVIDE(s.total_spend, s.total_orders) AS avg_order_value,

  sess.total_sessions AS total_sessions,
  sess.total_product_views AS total_product_views,
  sess.total_add_to_carts AS total_add_to_carts

FROM sales_year s
LEFT JOIN sessions_year sess
  ON s.year_number = sess.year_number
 AND s.customer_key_FK = sess.customer_key_FK;



    select*from prom_mart.MART_CUSTOMER_YEAR;
    select*from prom_mart.MART_DAILY_SALES;
